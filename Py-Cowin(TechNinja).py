...
img1 = Image.open('vaccinated image.png')
img1 = img1.resize((400,400))
st.image(img1,use_column_width=False)
st.title("Py-Cowin💉")
st.markdown("<h3 style='text-align: left; color: #518019;'>Vaccination Availability Checker(TechNinja)</h3>",
                unsafe_allow_html=True)
st.markdown("<h3 style='text-align: left; color: #87CEEB;'>A robust covid 19 data site with additional features coming very soon 😊.</h3>",
                unsafe_allow_html=True)
                ...
                if centers.empty:
                    ...
                    if new_df.empty:
                        ...
                        st.markdown(get_table_download_link(new_df,
                                                            district_box.replace(' ', '_') + '_' + new_date.replace('-',
                                                                                                                    '_') + '.csv',
                        ...
                        href_2 = f'<a href="{linkjson}">Some data may be incorrect so to click here to visit the source api data.</a>'
